    ...
    <div className="bg-slate-800 rounded-lg shadow-lg h-full flex flex-col">
      <div className="flex items-center justify-between p-4 border-b border-slate-700">
        ...
        <div className="flex items-center">
          <div className={`h-3 w-3 ${chatDisabled ? 'bg-red-500' : 'bg-green-500'} rounded-full mr-2`}></div>
          <span className="text-xs text-gray-400">{chatDisabled ? 'Offline' : 'Online'}</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 scrollbar-thin space-y-4">
        {chatDisabled ? (
          <div className="h-full flex flex-col items-center justify-center text-gray-400">
            ...
            <p className="text-center">Chat with Grid Agent is temporarily disabled</p>
            <p className="text-xs mt-1 text-center">Please use the interface controls instead</p>
          </div>
        ) : (
          <>
            {chatLogs.length === 0 ? (
      ...
      <div className="p-4 border-t border-slate-700">
        <div className="flex gap-2">
          <input
            ...
            onKeyDown={(e) => e.key === 'Enter' && !chatDisabled && handleSendMessage()}
            placeholder={chatDisabled ? "Chat is currently disabled" : "Type a message..."}
